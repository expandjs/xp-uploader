 ...
 */
module.exports = new XP.Class('XPUploader', {
    ...
    submit: {
        ...
        value(source, offset, callback) {
            ...
            // Checking
            if (!XP.isInstance(source, http ? stream.Readable : env.File)) { callback(new XP.ValidationError('data', http ? 'Readable' : 'File')); return; }
            if (!XP.isVoid(offset) && !XP.isInt(offset, true)) { callback(new XP.ValidationError('offset', 'number')); return; }
    ...
    _handleUpload(event) {
        ...
        // Emitting
        this.emit('upload', this.upLoaded += http ? XP.byteLength(event) : event.loaded, this.upTotal);
    }
});
...
// Globalizing
if (typeof window !== "undefined") { window.XPUploader = module.exports; }
